    ...
    */
	 ...
	 */
            ...
            double s, double sm, double s1, double sm1)
        {
            ...
            if (x == 0)
            {
                var ar = (rm1 - r1 + r - rm + (rm - r)/x)/(x - 1);
                var ass = (sm1 - s1 + s - sm + (sm - s)/x)/(x - 1);
                if (ass > 0 | ar < 0)
                {
	 ...
	 */
        ...
        public int GetPixelColour(Color pixel, int colour)
        {
            if (colour == ANALYSIS_COLOUR_RED)
                return pixel.R;
            if (colour == ANALYSIS_COLOUR_GREEN)
                return pixel.G;
            if (colour == ANALYSIS_COLOUR_BLUE)
                return pixel.B;
            return 0;
        }
        ...
        private Color[] FlipBlock(Color[] block, int[] mask)
        {
            ...
            for (var i = 0; i < block.Length; i++)
            {
                if (mask[i] == 1)
                {
                    ...
                    //build a new pixel
                    var newpixel = (0xff << 24) | ((red & 0xff) << 16)
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (var bitmap = new Bitmap(args[0]))
                {
                    ...
                    double average = 0;
                    var results = rsa.DoAnalysis(image, ANALYSIS_COLOUR_RED, true);
