...
namespace FunctionLib.Cryptography.Twofish
{
    public class TwofishBase
    {
        ...
        protected readonly int InputBlockSize = BLOCK_SIZE / 8;
        protected readonly int OutputBlockSize = BLOCK_SIZE / 8;
		...
		*
        ...
        protected bool ReKey(int keyLen, ref uint[] key32)
        {
            int i;
            mKeyLength = keyLen;
            mRounds = mNumRounds[(keyLen - 1) / 64];
            var subkeyCnt = ROUND_SUBKEYS + 2 * mRounds;
            var k32E = new uint[MAX_KEY_BITS / 64];
            var k32O = new uint[MAX_KEY_BITS / 64]; /* even/odd key dwords */

            var k64Cnt = (keyLen + 63) / 64;
            for (i = 0; i < k64Cnt; i++)
            {                       /* split into even/odd key dwords */
                k32E[i] = key32[2 * i];
                k32O[i] = key32[2 * i + 1];
                /* compute S-box keys using (12,8) Reed-Solomon code over GF(256) */
                mSboxKeys[k64Cnt - 1 - i] = RS_MDS_Encode(k32E[i], k32O[i]); /* reverse order */
            }

            for (i = 0; i < subkeyCnt / 2; i++)                 /* compute round subkeys for PHT */
            {
                var a = F32((uint)(i * SK_STEP), ref k32E, keyLen);
                var b = F32((uint)(i * SK_STEP + SK_BUMP), ref k32O, keyLen);
                b = ROL(b, 8);
                mSubKeys[2 * i] = a + b;         /* combine with a PHT */
                mSubKeys[2 * i + 1] = ROL(a + 2 * b, SK_ROTL);
            }
        ...
        protected void BlockDecrypt(ref uint[] x)
        {
            ...
            for (var i = 0; i < BLOCK_SIZE / 32; i++)   /* copy in the block, add whitening */
                x[i] ^= mSubKeys[OUTPUT_WHITEN + i];

            for (var r = mRounds - 1; r >= 0; r--)           /* main Twofish decryption loop */
            {
                var t0 = F32(x[0], ref mSboxKeys, mKeyLength);
                var t1 = F32(ROL(x[1], 8), ref mSboxKeys, mKeyLength);

                x[2] = ROL(x[2], 1);
                x[2] ^= t0 + t1 + mSubKeys[ROUND_SUBKEYS + 2 * r]; /* PHT, round keys */
                x[3] ^= t0 + 2 * t1 + mSubKeys[ROUND_SUBKEYS + 2 * r + 1];
                x[3] = ROR(x[3], 1);
        ...
        protected void blockEncrypt(ref uint[] x)
        {
            ...
            for (var r = 0; r < mRounds; r++)            /* main Twofish encryption loop */ // 16==rounds
            {
#if FEISTEL
...
#endif
                if (r < mRounds - 1)                     /* swap for next round */
                {
        ...
        private readonly int[] mNumRounds = { 0, ROUNDS_128, ROUNDS_192, ROUNDS_256 };
		...
		*
        ...
        private uint[] mSboxKeys = new uint[MAX_KEY_BITS / 64];    /* key bits used for S-boxes */
        private readonly uint[] mSubKeys = new uint[TOTAL_SUBKEYS];     /* round subkeys, input/output whitening bits */
        protected uint[] Key = { 0, 0, 0, 0, 0, 0, 0, 0 };              //new int[MAX_KEY_BITS/32];
        protected readonly uint[] IV = { 0, 0, 0, 0 };                       // this should be one block size
		...
		***********************************************************************/
        static private readonly byte[,] P8x8 =
        {
			/*  p0:   */
